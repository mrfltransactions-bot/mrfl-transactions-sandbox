...
| Operator dashboard | `dashboard/index.html` | Gloria's private all-transactions overview (v1.8.0): dark sidebar + stat chips + health donut + needs-attention rail + 7-day strip + pastel deal cards w/ milestone checklists + per-tab sheet links. Fed by `doGet?view=operator&key=<operator_key>` (Script Property; link via 🛠 TC Tools → "🖥 My dashboard link"). Same dark/light token system + contrast-audit rule as the portal; `ENDPOINT` const = webhook URL. |
