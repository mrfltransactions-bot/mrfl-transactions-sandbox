...
| Agent portal | `portal/index.html` | Mobile-first read-only app per agent: stats, next-deadline banner (tap-to-jump, red past-due variant), deal cards (progress, milestone timeline from sheet checkboxes, collapsible Contacts & details with tap-to-call/email), past deals collapsed. `PORTAL_ENDPOINT` const = Gloria's webhook URL. |
