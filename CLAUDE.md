...
| Operator dashboard | `dashboard/index.html` | Gloria's private all-transactions SPA (v2.5.0, aurora liquid-glass: violet-black bg + 3 vivid blobs, specular-edged frosted panels, glowing gradient ＋): hash-router views — #home (search, tappable stat cards, week day-pills w/ per-day deadline timeline, attention preview, **Quick links** card — external links to the intake form / public website / master sheet, each `target="_blank"`; consts `INTAKE_URL`/`SITE_URL`/`SHEET_URL` near `ENDPOINT`. **`INTAKE_URL` is the git-main Vercel origin** (`…-git-main-…vercel.app/intake/intake-form.html`), NOT the apex — the intake form keeps Gloria's Claude API key + webhook URL in per-origin localStorage, and she set those up on that origin; the apex is a different origin with empty localStorage, so don't "fix" it to mrfltransactions.com), #agents → #deals/agent/<name>, #deals/all|urgent|closing, #deal/<i> full detail (milestone checklist + ALL details sections + tel/mailto + Open-sheet-tab). Floating bottom nav; center ＋ opens the master sheet. Fed by `doGet?view=operator&key=<operator_key>` (v7.2 payload includes `details`; v7.6 adds `portal_links` map → 🔗 button on each Agents-tab row shares/copies that agent's portal invite). Link via 🛠 TC Tools → "🖥 My dashboard link". Detail view has "Open sheet tab ↗" + "🖨 Print / save as PDF" (same print pattern as the portal). Same dark/light token system + contrast-audit rule as the portal; `ENDPOINT` const = webhook URL. |
...
| Public site | `index.html` (root) + `refer/index.html` | Marketing site at `/` (services, how-it-works, pricing, contact CTAs) and the referral landing at `/refer/?from=<agent>` (personalized invite chip/copy/CTAs, $250/$350 offer). Canonical host: **https://mrfltransactions.com** — GoDaddy domain → Vercel, LIVE since 2026-07-06. Gloria set **www.mrfltransactions.com as primary**; the apex 307-redirects to it preserving path+query, so the apex-based generated links (portal REFER_BASE, Code.gs PORTAL/SITE/DASHBOARD_BASE_URL) all work — don't "fix" this. Machine-verify deploys against `https://www.mrfltransactions.com/` (apex curl returns a "Redirecting..." body). The vercel.app host stays attached so old shared links keep working. Hero: "Your deals, in good hands." (italic-serif em); closing CTA "Let's get your next deal moving."; refer-page closing CTA "<Referrer>'s deals are in good hands. Yours can be too." **"Lumi" UI** (v3.0.0, from Gloria's lumi.studio reference — her 5th style iteration: data-dashboard → NFT purple → liquid glass → ink & azure → this): bright, warm, playful — **light-first** (default light; dark toggle kept as a warm counterpart). Ivory bg #ffffff / soft #f7f7fb tint, one periwinkle-indigo accent #6950e6, warm-dark text #1a163d; **soft-serif display font Fraunces + grotesque body Manrope** (single Google Fonts link, both pages — Recoleta/Moderat substitutes; all h1/h2/h3 + price amounts are serif, everything else Manrope). A **pastel palette used as graphics/color-blocks** (salmon #fba297, gold #f9cf90, lilac #e5c6fb, sky #a3e3f5, mint #bdeeda, blush/cream) — homepage hero has a self-contained inline-SVG "live portal" illustration (house-chip card + milestone rows + progress bar + calendar chip + check badge, all `var()`-themed); service/bullet icon chips cycle through the 4 pastels via `:nth-child(4n+…)`; refer hero has two soft pastel blobs (`.hero::before/::after`). Very rounded: 100px pill buttons (periwinkle bg + WHITE text — white on #6950e6 = 5.4:1, passes), 22–26px cards, eyebrow pill labels. No azure bars — h2s are just big Fraunces. Accessibility gotchas locked in: **light `--star` must be dark amber #b45309** (gold #f0972a on white = 2.29:1, fails — stars convey rating); **light `--muted` = #67657a** (#767488 dropped to 4.25:1 on the #f7f7fb tint); **dark `--acc-soft` is SOLID #241d44** not translucent (light-purple eyebrow/step-number text needs an opaque chip to pass). Emoji flags in the contrast audit are false positives (color glyphs, not monochrome text). Toggle persisted in localStorage `mrfl_site_theme` (shared by both pages); theme-color meta #ffffff / #151129. Copy rules: say "double sided" (never "dual agency"); no "deals close on time"-style absolute claims. ⚠️ vercel.json's old `/` → intake redirect was REMOVED — Gloria's intake bookmark must be `/intake/intake-form.html`. |
